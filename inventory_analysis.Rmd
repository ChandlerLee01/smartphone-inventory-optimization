...
plot_data <- metrics_df |>
  filter(Metric %in% c("Avg Inventory", "Monthly Turnover", "Stockout Days")) |>
  pivot_longer(cols = c(Original, Reduced_SS, Better_Acc),
               names_to = "Scenario",
               values_to = "Value") |>
  mutate(
    Scenario = factor(Scenario,
                      levels = c("Original", "Reduced_SS", "Better_Acc"),
                      labels = c("Before", "Reduced Safety Stock", "Better Forecast Accuracy")),
    Metric = factor(Metric,
                    levels = c("Avg Inventory", "Monthly Turnover", "Stockout Days"))
  )

ggplot(plot_data, aes(x = Metric, y = Value, fill = Scenario)) +
  geom_col(position = "dodge") +
  labs(
    title = "Impact of Process Improvements on Inventory Performance",
    x = "Metric",
    y = "Value"
  ) +
  theme_minimal()

ggsave("plots/improvement_impact.png", width = 8, height = 5)
```
**Interpretation:**
